...
hostnamectl --static set-hostname jdbc-http
...
sed -i "s/^hostname=localhost/hostname=jdbc-http.hdp.local/g" /etc/ambari-agent/conf/ambari-agent.ini
...
curl --silent --show-error -H "X-Requested-By: ambari" -X POST -d '@/tmp/install/blueprint.json' -u admin:admin http://jdbc-http.hdp.local:8080/api/v1/blueprints/generated
curl --silent --show-error -H "X-Requested-By: ambari" -X POST -d '@/tmp/install/create-cluster.json' -u admin:admin http://jdbc-http.hdp.local:8080/api/v1/clusters/jdbc-http
...
until [ $PROGRESS -eq 100 ]; do
    PROGRESS=`curl --silent --show-error -H "X-Requested-By: ambari" -X GET -u admin:admin http://jdbc-http.hdp.local:8080/api/v1/clusters/jdbc-http/requests/1 2>&1 | grep -oP '\"progress_percent\"\s+\:\s+\K[0-9]+'`
